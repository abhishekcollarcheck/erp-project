...
export class AttendanceMSSQLService {
  ...
  async getSampleFromTable(tableName: string): Promise<{
    ...
    const validTables = await queryMSSQL<{ TableName: string }>(`
      ...
      WHERE TABLE_TYPE = 'BASE TABLE'
    `);
    const match = validTables.find(
      (t) => t.TableName.toLowerCase() === tableName.toLowerCase(),
    );
    if (!match)
      throw new Error(`Table "${tableName}" not found in Realtime database`);
    ...
    const [columns, countResult, rows] = await Promise.all([
      queryMSSQL<{ ColumnName: string }>(`
        ...
        ORDER BY ORDINAL_POSITION
      `),
      queryMSSQL<{ Total: number }>(
        `SELECT COUNT(*) AS Total FROM [${realName}]`,
      ),
  ...
  async getAttendanceByDate(
    ...
    const rows = await queryMSSQL<RawAggregatedRow>(
     ...
     CONVERT(
         VARCHAR(8),
           CASE 
             WHEN COUNT(*) > 1 THEN MAX(${COL.punchDatetime})
         ELSE NULL
         END,
         108
            ) AS last_out,
  ...
  private mapAggregatedRow(r: RawAggregatedRow): MSSQLAttendanceRow {
    ...
    if (r.first_in && r.last_out) {
      working_hours = this.calculateWorkingHours(r.first_in, r.last_out);
    }
  ...
  private calculateWorkingHours(checkIn: string, checkOut: string): number {
    ...
    let inSeconds = ih * 3600 + im * 60 + is;

    let outSeconds = oh * 3600 + om * 60 + os;
