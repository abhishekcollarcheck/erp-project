...
// /**
//  * Returns null for a "Full Attendance" shift (start_time and/or end_time is
//  * null on the Shift row) — there is no fixed window to derive boundaries
//  * from, so callers must branch on the FULL_ATTENDANCE path instead of
//  * treating this as a normal timed shift.
//  *
//  * `day_span` ('1 day' | '2 days') replaces the old `crosses_midnight`
//  * boolean. `duration_minutes` no longer exists on the model, so duration is
//  * derived from start/end here instead of read off the row. `half_day_time`
//  * is now a first-class column (previously an ad-hoc `(shift as any)` read of
//  * a not-yet-existing `half_shift_time` field) — used directly, with the old
//  * midpoint fallback kept only for when it's null.
//  */
// function computeShiftBoundaries(shift: Shift, graceMinutes: number): ShiftBoundariesSeconds | null {
//   if (!shift.start_time || !shift.end_time) {
//     return null;
//   }

//   const start = parseTimeToSeconds(shift.start_time);
//   let end = parseTimeToSeconds(shift.end_time);

//   if (shift.day_span === '2 days' && end <= start) {
//     end += 86400;
//   }

//   const durationSec = end - start;

//   const halfShift = shift.half_day_time
//     ? parseTimeToSeconds(shift.half_day_time)
//     : start + durationSec / 2;
...
//   const inSec = parseTimeToSeconds(checkIn);
//   let outSec = parseTimeToSeconds(checkOut);
//   if (shift.day_span === '2 days' && outSec < inSec) {
//     outSec += 86400;
//   }
//   return { inSec, outSec };
// }
...
//   // ── 3a. FULL_ATTENDANCE SHIFT (start_time and/or end_time is null) ──
//   // There's no scheduled window to judge lateness, half-day, or duration
//   // thresholds against, so none of the boundary-based logic below applies.
//   //
//   // ASSUMPTION (unconfirmed — flag if wrong): a complete in/out punch pair
//   // is simply PRESENT, on a week-off/holiday it's treated as worked-on-the-
//   // day-off (POW/POH), and there is no half-day/miss-punch distinction for
//   // this shift type. Adjust this block once the intended rule is confirmed.
//   if (b === null) {
//     if (isWeekOff) {
//       return { status: 'PRESENT_ON_WEEK_OFF', matchedRule: 'FULL_ATTENDANCE_ANY_PUNCH_PAIR_WO', lateMinutes: 0 };
//     }
//     if (isHoliday) {
//       return { status: 'PRESENT_ON_HOLIDAY', matchedRule: 'FULL_ATTENDANCE_ANY_PUNCH_PAIR_HO', lateMinutes: 0 };
//     }
//     return { status: 'PRESENT', matchedRule: 'FULL_ATTENDANCE_ANY_PUNCH_PAIR', lateMinutes: 0 };
//   }
