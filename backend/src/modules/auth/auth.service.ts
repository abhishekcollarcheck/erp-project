...
export async function countCompanySuperAdmins(
  ...
  const saRole = await Role.findOne({
    where: { company_id: companyId, slug: "super_admin" },
  });
  if (!saRole) return 0;
...
if (
  ...
  const managedCompanies = await Promise.all(
    assignments.map(async (a) => {
      ...
      const companyPayload = await buildPayload(employee, a.company_id);
      ...
      return {
        ...
        is_super_admin: isSA,
        // active_modules,
        role_id: companyPayload.roleId,
        role_slug: companyPayload.roleSlug,
        permissions: companyPayload.permissions,        
      };
    }),
  );
...
export class AuthService {
  async requestOtp(
    ...
    if (employee.otp_locked_until && new Date() < employee.otp_locked_until) {
      const mins = Math.ceil(
        (employee.otp_locked_until.getTime() - Date.now()) / 60000,
      );
      throw new AppError(
    ...
    if (recentCount >= OTP_RATE_LIMIT)
      throw new AppError("Too many OTP requests. Wait 1 hour.", 429);
  ...
  async verifyOtp(emailOrPhone: string, otp: string, ipAddress?: string) {
    ...
    if (!employee.portal_access)
      throw new AppError("Portal access disabled. Contact HR.", 403);
    ...
    if (!employee.otp_hash || !employee.otp_expires)
      throw new AppError("No OTP found. Request a new one.", 400);
    ...
    await OtpRequest.update(
      { used_at: new Date() },
      { where: { employee_id: employee.id, used_at: null }, limit: 1 },
    );
  ...
  async refresh(
    ...
    if (!employee.portal_access)
      throw new AppError("Portal access disabled.", 403);
