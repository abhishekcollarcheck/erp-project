...
async function postEarnedLeave({
  ...
  console.log('[EL POSTING] Employee:', employeeId);
  console.log('[EL POSTING] Month:', `${year}-${month}`);
  console.log('[EL POSTING] Earned:', leave.earned_days);

  const earnedDays = Number(leave.earned_days || 0);

  if (earnedDays <= 0) {
    console.log('[EL POSTING] No EL to post');
    return null;
  }
  ...
  console.log(
    '[EL POSTING] On probation:',
    probation.on_probation
  );

  console.log(
    '[EL POSTING] Probation status:',
    probation.probation_status
  );

  console.log(
    '[EL POSTING] Currently on probation:',
  ...
  if (isCurrentlyOnProbation) {
    ...
    console.log(
      '[EL POSTING] Earned this month:',
      earnedDays
    );

    console.log(
      '[EL POSTING] New probation EL credit:',
      newCredit
    );
