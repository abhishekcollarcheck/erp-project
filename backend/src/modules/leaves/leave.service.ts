...
//     probation_el_transfered: number;
//   };
...
//    *
//    * on_probation = true
//    *     -> employee is currently on probation
//    *
//    * on_probation = false
//    *     -> probation is completed
//    *
//    * If on_probation is true but probation_end_date has already passed,
//    * we also consider probation completed.
//    */

//   let currentlyOnProbation = false;
//   let probationCompleted = false;
...
//     const probationEndDate = probationData.probation_end_date
//       ? new Date(probationData.probation_end_date)
//       : null;
...
//     if (onProbation) {
//       // No end date -> trust on_probation flag
//       if (!probationEndDate) {
//         currentlyOnProbation = true;
//         probationCompleted = false;
//       } else {
//         const endTime = probationEndDate.getTime();
//         const processingTime = processingDate.getTime();

//         // Invalid end date -> trust on_probation flag
//         if (Number.isNaN(endTime)) {
//           currentlyOnProbation = true;
//           probationCompleted = false;
//         }

//         // End date reached/passed -> probation completed
//         else if (processingTime >= endTime) {
//           currentlyOnProbation = false;
//           probationCompleted = true;
//         }

//         // End date is still in future -> currently on probation
//         else {
//           currentlyOnProbation = true;
//           probationCompleted = false;
//         }
...
//       // Final calculated probation status
//       on_probation: currentlyOnProbation,
...
//         Number(
//           probationData?.probation_el_transefered ?? 0,
//         ),
//     },
...
//     grace_minutes: number | null;
//   };
//   // CORRECTED — this block previously mirrored the OLD probation model
//   // (probation_final_status / confirmation_status / confirmed_on /
//   // probation_extended_period), none of which exist on the new
//   // EmployeeCommitmentProbation model anymore. The new model only tracks
//   // on_probation + probation_end_date (+ period/status/EL credit fields),
//   // so this interface is trimmed to match exactly what
//   // getEmployeeLeaveInformation() below actually returns.
//   probation: {
//     exists: boolean;
//     on_probation: boolean;
//     probation_period: string | null; // model column is STRING(30), not a number
//     probation_end_date: string | null;
//     probation_status: string | null;
//     probation_el_credit: number;
//     probation_el_transefered: number; // spelled to match the model column exactly
//   };
//   leave_status: EmployeeLeaveStatus;
//   joining_date: string | null;
//   probation_completed: boolean;
...
//   // ==========================================================================
//   // GET PROBATION RECORD
//   // ==========================================================================
//   // CORRECTED — EmployeeCommitmentProbation's primary key is employee_id
//   // (see the model: `employee_id: { primaryKey: true }`), not a separate
//   // auto-increment `id` column. There is exactly one row per employee, so
//   // there's no "latest" to order for — the old `order: [['id', 'DESC']]`
//   // referenced a column that doesn't exist on this table and would throw
//   // (or silently fail depending on dialect/strictness). findOne on the PK
...
//   /*
//    * Probation logic (matches the new EmployeeCommitmentProbation model,
//    * which only tracks on_probation + probation_end_date — no more
//    * confirmation_status / probation_final_status):
//    *
//    * on_probation = true
//    *     -> employee is currently on probation
//    *
...
//    *
//    * If on_probation is true but probation_end_date has already passed,
//    * we also consider probation completed (end date wins over the flag
...
//   console.log("Record", record , "Employee Id ", employeeId , "year", year , "month" , month);
...
export interface EmployeeLeaveInformation {
  ...
  probation: {
    ...
    probation_status: string | null;
    probation_el_credit: number;
    probation_el_transefered: number;
  };
  leave_status: EmployeeLeaveStatus;
  joining_date: string | null;
  probation_completed: boolean;
...
// All fields optional — edit() falls back to the existing value on the
// leave request for anything the caller doesn't send.
...
export class LeaveService {
  ...
  // FIXED — this used to ignore the managerId entirely and return every
  // Pending request in the company. Now it only returns requests where the
  // given manager is stored as l1_manager_id OR l2_manager_id on the
  // request (both fields already captured at apply()/edit() time), so an
  // employee's leave shows up for BOTH their L1 and L2 manager.
  ...
  // ─── Single leave request, full detail ──────────────────────────────────
  async getById(id: number, companyId: number) {
    const leave = await LeaveRequest.findOne({
      where: { id },
      include: [
  ...
  // ─── Per-day sandwich/charged breakdown ─────────────────────────────────
  async getBreakdown(leaveRequestId: number) {
    const exists = await LeaveRequest.findByPk(leaveRequestId, { attributes: ['id'] });
    if (!exists) throw new AppError('Leave request not found', 404);
  ...
  async apply(dto: ApplyLeaveDto, companyId: number) {
    ...
    await this.checkOverlap(dto.employee_id, dto.from_date, dto.to_date, dto.leave_application_type);

    const { l1ManagerId, l2ManagerId } = await this.resolveAndValidateManagers(
      dto.employee_id,
      dto.l1_manager_id,
      dto.l2_manager_id,
      companyId,
    ...
    if (leaveType.code === 'SHORT') {
      ...
      return sequelize.transaction(async (transaction) => {
        await this.placeHoldForEdit(
          { employee_id: dto.employee_id, leave_type: leaveType, from_date: dto.from_date, days: dto.days, minutes },
          transaction,
        );
        return this.createLeaveRequestWithRefNo(leaveRequestPayload, transaction);
      });
    ...
    if (BALANCE_TRACKED_LEAVE_CODES.includes(leaveType.code)) {
      return sequelize.transaction(async (transaction) => {
        await this.placeHoldForEdit(
          { employee_id: dto.employee_id, leave_type: leaveType, from_date: dto.from_date, days: dto.days, minutes: dto.minutes ?? 0 },
          transaction,
        );
        return this.createLeaveRequestWithRefNo(leaveRequestPayload, transaction);
      });
    ...
    // Everything else falls through with no balance check — the explicit
    // "untracked" path for a leave type marked as unlimited/unmetered.
    return this.createLeaveRequestWithRefNo(leaveRequestPayload);
  }

  // ─── Edit a leave request ────────────────────────────────────────────────
  // Owner-only, and only while the request is still Pending — once it's
  // Approved the balance has already moved from `pending` to `used`, so
  // editing an approved leave should go through cancel + re-apply instead.
  // Re-validates managers only if the caller actually changed them, and
  // always re-checks overlap (excluding this leave itself) and the balance
  // hold, since date/days/leave-type/minutes may all change.
  async edit(id: number, employeeId: number, dto: EditLeaveDto, companyId: number) {
    const leave = await this.findByIdScoped(id, companyId);
    ...
    const oldLeaveTypeCode = (leave as any).leaveType?.code as string | undefined;

    const leaveTypeId = dto.leave_type_id ?? leave.leave_type_id;
    const newLeaveType = await LeaveType.findOne({
      where: { id: leaveTypeId, company_id: companyId, is_active: true },
    });
    if (!newLeaveType) throw new AppError('Leave type not found or inactive', 404);

    const leaveApplicationType = dto.leave_application_type ?? leave.leave_application_type;
    const fromDate = dto.from_date ?? leave.from_date;
    const toDate = dto.to_date ?? leave.to_date;
    const fromTime = dto.from_time !== undefined ? dto.from_time : leave.from_time;
    const toTime = dto.to_time !== undefined ? dto.to_time : leave.to_time;
    const days = dto.days ?? Number(leave.days);
    const minutes = dto.minutes ?? Number(leave.minutes);
    const reason = dto.reason !== undefined ? dto.reason : leave.reason;
    const undertakingAccepted = dto.undertaking_accepted ?? leave.undertaking_accepted;

    if (SHORT_LEAVE_APPLICATION_TYPES.includes(leaveApplicationType) && (!fromTime || !toTime)) {
      throw new AppError('From/To time are required for Arrival Late / Leaving Early', 400);
    }

    let l1ManagerId = leave.l1_manager_id;
    let l2ManagerId = leave.l2_manager_id;
    if (dto.l1_manager_id !== undefined || dto.l2_manager_id !== undefined) {
      const resolved = await this.resolveAndValidateManagers(
        leave.employee_id,
        dto.l1_manager_id !== undefined ? dto.l1_manager_id : leave.l1_manager_id,
        dto.l2_manager_id !== undefined ? dto.l2_manager_id : leave.l2_manager_id,
        companyId,
      );
      l1ManagerId = resolved.l1ManagerId;
      l2ManagerId = resolved.l2ManagerId;
    }

    await this.checkOverlap(leave.employee_id, fromDate, toDate, leaveApplicationType, leave.id);

    return sequelize.transaction(async (transaction) => {
      // release whatever hold the OLD version of this request had, using
      // the leave instance's still-unmutated (pre-update) values
      if (oldLeaveTypeCode) {
        await this.releaseHoldForEdit(leave, oldLeaveTypeCode, transaction);
      }

      // validate + place a hold under the NEW values
      await this.placeHoldForEdit(
        { employee_id: leave.employee_id, leave_type: newLeaveType, from_date: fromDate, days, minutes },
        transaction,
      );
  ...
  // ─── Approve ───────────────────────────────────────────────────────────────
  // FIXED — previously any user with the 'leaves:approve' permission could
  // approve ANY employee's leave, company-wide. Now the caller must be the
  // L1 or L2 manager stored on this specific leave request — either one can
  // approve it (first responder decides) — unless isOverride is set (HR /
  // superadmin), which is resolved from permissions in the controller.
  async approve(id: number, approvedBy: number, companyId: number, isOverride = false) {
    const leave = await this.findByIdScoped(id, companyId);
    if (leave.status !== 'Pending')
      throw new AppError('Only Pending requests can be approved', 400);

    if (!isOverride && leave.l1_manager_id !== approvedBy && leave.l2_manager_id !== approvedBy) {
      throw new AppError('You are not a manager on this leave request', 403);
    }
  ...
  // ─── Reject ────────────────────────────────────────────────────────────────
  // Same manager-match fix as approve() above.
  async reject(id: number, rejectedBy: number, companyId: number, reason: string | undefined, isOverride = false) {
    const leave = await this.findByIdScoped(id, companyId);
    if (leave.status !== 'Pending')
      throw new AppError('Only Pending requests can be rejected', 400);

    if (!isOverride && leave.l1_manager_id !== rejectedBy && leave.l2_manager_id !== rejectedBy) {
      throw new AppError('You are not a manager on this leave request', 403);
    }
  ...
  // ─── New shared helpers (used by both apply() and edit()) ──────────────

  // Validates the l1/l2 manager ids the caller sent against the employee's
  // actual manager mapping in EmployeeManagersWorkContact, and confirms
  // both managers belong to the same company.
  private async resolveAndValidateManagers(
    employeeId: number,
    l1: number | null | undefined,
    l2: number | null | undefined,
    companyId: number,
  ): Promise<{ l1ManagerId: number | null; l2ManagerId: number | null }> {
    const l1ManagerId = l1 === null || l1 === undefined ? null : Number(l1);
    const l2ManagerId = l2 === null || l2 === undefined ? null : Number(l2);
    ...
    const actualL1ManagerId = managerMapping?.l1_manager_id ? Number(managerMapping.l1_manager_id) : null;
    const actualL2ManagerId = managerMapping?.l2_manager_id ? Number(managerMapping.l2_manager_id) : null;
    ...
    const managerIds = [l1ManagerId, l2ManagerId].filter((id): id is number => id !== null);
    if (managerIds.length) {
      const managerCount = await Employee.count({
        where: { id: { [Op.in]: managerIds }, company_id: companyId },
      });
      if (managerCount !== new Set(managerIds).size) {
        throw new AppError('One or more managers do not belong to this company', 400);
      }
    ...
    return { l1ManagerId, l2ManagerId };
  }

  // Checks for overlapping Pending/Approved leave for the same employee.
  // excludeLeaveId lets edit() ignore the request being edited itself.
  private async checkOverlap(
    employeeId: number,
    fromDate: string,
    toDate: string,
    applicationType: LeaveApplicationType,
    ...
    const newIsHalfDay = HALF_DAY_APPLICATION_TYPES.includes(applicationType) && fromDate === toDate;

    const overlap = candidates.find((existing) => {
      const existingIsHalfDay = HALF_DAY_APPLICATION_TYPES.includes(existing.leave_application_type)
        && existing.from_date === existing.to_date;
      const sameSingleDay = newIsHalfDay && existingIsHalfDay && existing.from_date === fromDate;
      const differentHalves = existing.leave_application_type !== applicationType;
      if (sameSingleDay && differentHalves) return false;
      return true;
    });
  ...
  // Releases a Pending hold placed under the OLD leave-type code, using the
  // leave instance's values as they still are BEFORE leave.update() runs.
  private async releaseHoldForEdit(leave: LeaveRequest, oldLeaveTypeCode: string, transaction: Transaction) {
    if (oldLeaveTypeCode === 'SHORT') {
      const balance = await this.getMinutesBalanceRowForLeave(leave, transaction);
      if (!balance) return;
      balance.pending_minutes = Math.max(0, Number(balance.pending_minutes) - Number(leave.minutes));
      await balance.save({ transaction });
    ...
    if (BALANCE_TRACKED_LEAVE_CODES.includes(oldLeaveTypeCode)) {
      const balance = await this.getBalanceRowForLeave(leave, transaction);
      if (!balance) return;
      balance.pending = Math.max(0, Number(balance.pending) - Number(leave.days));
      await balance.save({ transaction });
  ...
  // Validates sufficient balance and places a Pending hold under the NEW
  // values. Shared by apply() and edit().
  private async placeHoldForEdit(
    params: { employee_id: number; leave_type: LeaveType; from_date: string; days: number; minutes: number },
    transaction: Transaction,
  ) {
    const { employee_id, leave_type: leaveType, from_date, days, minutes } = params;

    if (leaveType.code === 'SHORT') {
      if (minutes <= 0) throw new AppError('minutes is required for Short Leave', 400);
      const chunk = Number(leaveType.split_chunk_minutes) || 30;
      const quota = Number(leaveType.monthly_quota_minutes) || 60;
      const validAmount = minutes === quota || (leaveType.allow_split && minutes === chunk);
      if (!validAmount) {
      ...
      const [balance] = await EmployeeLeaveMinutesBalance.findOrCreate({
        where: { employee_id, leave_type_id: leaveType.id, year, month },
        defaults: {
          employee_id,
          leave_type_id: leaveType.id,
          year,
      ...
      const available = Number(balance.allocated_minutes) - Number(balance.used_minutes) - Number(balance.pending_minutes);
      if (minutes > available) {
        throw new AppError(
          `Insufficient Short Leave balance: ${available} minute(s) remaining this month, requested ${minutes}`,
          400,
    ...
    if (BALANCE_TRACKED_LEAVE_CODES.includes(leaveType.code)) {
      ...
      const [balance] = await EmployeeLeaveBalance.findOrCreate({
        where: { employee_id, leave_type_id: leaveType.id, year },
        defaults: {
          employee_id,
          leave_type_id: leaveType.id,
          year,
      ...
      const available = Number(balance.allocated) + Number(balance.carried_forward) - Number(balance.used) - Number(balance.pending);
      if (days > available) {
        throw new AppError(
          `Insufficient ${leaveType.name} balance: ${available} day(s) remaining, requested ${days}`,
          400,
        );
      }
    ...
    // untracked leave types: no balance row to touch
  }
}

// ─── Standalone exports (unchanged) ─────────────────────────────────────────
...
export async function getEmployeeLeaveInformation(
  ...
  return {
    ...
    probation: {
      ...
      probation_status: probationData?.probation_status ?? null,
      probation_el_credit: Number(probationData?.probation_el_credit ?? 0),
      probation_el_transefered: Number(probationData?.probation_el_transefered ?? 0),
    },
