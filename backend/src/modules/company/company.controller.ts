...
/**
 * Shared validation for employee-code range + skip list.
 * Used by both createCompany and updateCompany so an update can't
 * silently bypass the checks that create enforces.
 *
 * Returns { startCode, endCode, skipCodes } on success, or null after
 * having already sent an error response.
 */
function validateEmployeeCodeRange(
  req: Request,
  res: Response,
  body: {
    employee_code_start?: any;
    employee_code_end?: any;
    employee_code_skip?: any;
  }
): { startCode: number | null; endCode: number | null; skipCodes: number[] } | null {
  const startCode = body.employee_code_start !== undefined && body.employee_code_start !== null
    ? Number(body.employee_code_start)
    : null;

  const endCode = body.employee_code_end !== undefined && body.employee_code_end !== null
    ? Number(body.employee_code_end)
    : null;
  ...
  if (startCode !== null && endCode !== null && startCode >= endCode) {
    sendError(res, 'Employee code end must be greater than start', 400);
    return null;
  }
  ...
  for (const code of skipCodes) {
    ...
    if (startCode !== null && endCode !== null && (code < startCode || code > endCode)) {
      sendError(res, `Reserved code ${code} is outside company range`, 400);
      return null;
    }
  }

  return { startCode, endCode, skipCodes };
}
...
async function createCompany(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const {
      ...
      // ── Newly wired-up fields (previously only on the model, not accepted here) ──
      code,
      logo_url,
      gstin,
      pan,
    ...
    if (!codeValidation) return; // error already sent
    const { startCode, endCode, skipCodes } = codeValidation;
    ...
    if (code) {
      const codeExists = await Company.findOne({ where: { code }, paranoid: false });
      if (codeExists) { sendError(res, 'A company with this code already exists', 409); return; }
    }
    ...
    try {
      ...
      const company = await Company.create({
        ...
        country: country || undefined, // let model default ('India') apply if omitted
        fiscal_year: fiscal_year || undefined, // model default ('Apr-Mar')
        timezone: timezone || undefined, // model default ('Asia/Kolkata')
        currency: currency || undefined, // model default ('INR')
        date_format: date_format || undefined, // model default ('DD/MM/YYYY')
...
async function updateCompany(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // ── Full field whitelist (all client-editable Company model fields) ──
    const allowed = [
      'name', 'slug', 'code',
      'city', 'state', 'country', 'address', 'pincode',
      'industry', 'email', 'phone', 'website', 'logo_url',
      'gstin', 'pan', 'cin',
      'timezone', 'currency', 'date_format', 'fiscal_year',
      'notes', 'theme_color',
      'legal_name', 'tagline', 'since_year', 'google_maps_link', 'hr_email', 'about',
    ];
    const updates: any = {};
    for (const k of allowed) { if (req.body[k] !== undefined) updates[k] = req.body[k]; }
    ...
    if (updates.slug && updates.slug !== company.slug) {
      const slugExists = await Company.findOne({ where: { slug: updates.slug, id: { [Op.ne]: company.id } }, paranoid: false });
      if (slugExists) { sendError(res, 'A company with this slug already exists', 409); return; }
    }
    if (updates.code && updates.code !== company.code) {
      const codeExists = await Company.findOne({ where: { code: updates.code, id: { [Op.ne]: company.id } }, paranoid: false });
      if (codeExists) { sendError(res, 'A company with this code already exists', 409); return; }
    }

    // employee code range/skip — validated the same way as on create, applied separately
    // since they need cross-field validation against each other (and against
    // existing DB values when only one of start/end/skip is being changed)
    if (
      req.body.employee_code_start !== undefined ||
      req.body.employee_code_end !== undefined ||
      req.body.employee_code_skip !== undefined
    ) {
      const codeValidation = validateEmployeeCodeRange(req, res, {
        employee_code_start: req.body.employee_code_start ?? company.employee_code_start,
        employee_code_end: req.body.employee_code_end ?? company.employee_code_end,
        employee_code_skip: req.body.employee_code_skip ?? company.employee_code_skip,
      });
      if (!codeValidation) return; // error already sent
      const { startCode, endCode, skipCodes } = codeValidation;
      updates.employee_code_start = startCode !== null ? String(startCode) : null;
      updates.employee_code_end = endCode !== null ? String(endCode) : null;
      updates.employee_code_skip = JSON.stringify(skipCodes);
    }
