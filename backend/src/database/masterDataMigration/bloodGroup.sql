-- ============================================================================
--  Blood Group form definition — MySQL
--  Same pattern as Gender/Marital Status: one hr_modules row, one
...
--  model but isn't exposed in BloodGroupPage.tsx's UI, so excluded.
--  max_length 50 matches BloodGroup.name (STRING(50)).
--
...
VALUES
    ('Blood Group', 'blood_group', '🩸', 'Blood group catalog', 16, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'blood_group' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Blood Group', 'blood_group', 'Blood group catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'blood_group' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Blood Group Name', 'blood_group_name', 'Blood Group',
     'Add blood group...', NULL,
     1, 50, NULL, NULL, NULL,
     NULL, 0, 1, 1, NOW(), NOW())
...
SELECT module, action, slug, description FROM (
    SELECT 'blood_group' AS module, 'view'     AS action, 'blood_group:view'     AS slug, NULL AS description
    UNION ALL SELECT 'blood_group', 'create',   'blood_group:create',   NULL
    UNION ALL SELECT 'blood_group', 'edit',     'blood_group:edit',     NULL
    UNION ALL SELECT 'blood_group', 'delete',   'blood_group:delete',   NULL
    UNION ALL SELECT 'blood_group', 'download', 'blood_group:download', NULL
) AS new_perms
WHERE NOT EXISTS (
