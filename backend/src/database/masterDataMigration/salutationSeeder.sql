-- ============================================================================
--  Salutation form definition — MySQL
--  Same pattern as Emergency Relationship/Gender: one hr_modules row, one
...
--  model but isn't exposed in SalutationPage.tsx's UI, so excluded.
--  max_length 50 matches Salutation.name (STRING(50)).
--
...
VALUES
    ('Salutation', 'salutation', '🎩', 'Salutation catalog (Mr., Mrs., Dr., etc.)', 24, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'salutation' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Salutation', 'salutation', 'Salutation catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'salutation' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Salutation Name', 'salutation_name', 'Salutation',
     'Add salutation...', NULL,
     1, 50, NULL, NULL, NULL,
     NULL, 0, 1, 1, NOW(), NOW())
...
SELECT module, action, slug, description FROM (
    SELECT 'salutation' AS module, 'view'     AS action, 'salutation:view'     AS slug, NULL AS description
    UNION ALL SELECT 'salutation', 'create',   'salutation:create',   NULL
    UNION ALL SELECT 'salutation', 'edit',     'salutation:edit',     NULL
    UNION ALL SELECT 'salutation', 'delete',   'salutation:delete',   NULL
    UNION ALL SELECT 'salutation', 'download', 'salutation:download', NULL
) AS new_perms
WHERE NOT EXISTS (
