-- ============================================================================
--  Bank form definition — MySQL
--  Same pattern as Salutation/Gender: one hr_modules row, one
...
--  model but isn't exposed in AllBanksPage.tsx's UI, so excluded.
--  max_length 150 matches Bank.name (STRING(150) — longer than the usual 50/100).
--
...
VALUES
    ('Bank', 'bank', '🏦', 'Bank catalog for employee bank account details', 25, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'bank' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Bank', 'bank', 'Bank catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'bank' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Bank Name', 'bank_name', 'Bank',
     'Add all banks...', NULL,
...
SELECT module, action, slug, description FROM (
    SELECT 'bank' AS module, 'view'     AS action, 'bank:view'     AS slug, NULL AS description
    UNION ALL SELECT 'bank', 'create',   'bank:create',   NULL
    UNION ALL SELECT 'bank', 'edit',     'bank:edit',     NULL
    UNION ALL SELECT 'bank', 'delete',   'bank:delete',   NULL
    UNION ALL SELECT 'bank', 'download', 'bank:download', NULL
) AS new_perms
WHERE NOT EXISTS (
