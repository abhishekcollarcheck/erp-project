-- ============================================================================
--  Qualification form definition — MySQL
...
--  model but isn't exposed in QualificationPage.tsx's UI, so excluded.
--  max_length 100 matches Qualification.name (STRING(100)).
--
...
VALUES
    ('Qualification', 'qualification', '🎓', 'Educational qualification catalog', 20, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'qualification' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Qualification', 'qualification', 'Qualification catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'qualification' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Qualification Name', 'qualification_name', 'Qualification',
...
SELECT module, action, slug, description FROM (
    SELECT 'qualification' AS module, 'view'     AS action, 'qualification:view'     AS slug, NULL AS description
    UNION ALL SELECT 'qualification', 'create',   'qualification:create',   NULL
    UNION ALL SELECT 'qualification', 'edit',     'qualification:edit',     NULL
    UNION ALL SELECT 'qualification', 'delete',   'qualification:delete',   NULL
    UNION ALL SELECT 'qualification', 'download', 'qualification:download', NULL
) AS new_perms
WHERE NOT EXISTS (
