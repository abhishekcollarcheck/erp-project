-- ============================================================================
--  Nationality form definition — MySQL
--  Same pattern as Gender/Religion: one hr_modules row, one
...
--  model but isn't exposed in NationalityPage.tsx's UI, so excluded.
--  max_length 50 matches Nationality.name (STRING(50)).
--
...
VALUES
    ('Nationality', 'nationality', '🌍', 'Nationality catalog', 18, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'nationality' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Nationality', 'nationality', 'Nationality catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'nationality' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Nationality Name', 'nationality_name', 'Nationality',
...
SELECT module, action, slug, description FROM (
    SELECT 'nationality' AS module, 'view'     AS action, 'nationality:view'     AS slug, NULL AS description
    UNION ALL SELECT 'nationality', 'create',   'nationality:create',   NULL
    UNION ALL SELECT 'nationality', 'edit',     'nationality:edit',     NULL
    UNION ALL SELECT 'nationality', 'delete',   'nationality:delete',   NULL
    UNION ALL SELECT 'nationality', 'download', 'nationality:download', NULL
) AS new_perms
WHERE NOT EXISTS (
