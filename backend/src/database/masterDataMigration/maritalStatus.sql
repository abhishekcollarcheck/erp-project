-- ============================================================================
--  Marital Status form definition — MySQL
--  Same pattern as Gender: one hr_modules row, one module_companies
--  enablement row, one form_definition, one field.
--
--  Only `name` is modeled as a permissioned field — `code` exists on the
--  model but isn't exposed in MaritalStatusPage.tsx's UI, so excluded.
--  max_length 50 matches MaritalStatus.name (STRING(50)).
--
...
VALUES
    ('Marital Status', 'marital_status', '💍', 'Marital status catalog', 15, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'marital_status' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Marital Status', 'marital_status', 'Marital status catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'marital_status' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Marital Status Name', 'marital_status_name', 'Marital Status',
     'Add marital status...', NULL,
     1, 50, NULL, NULL, NULL,
     NULL, 0, 1, 1, NOW(), NOW())
...
SELECT module, action, slug, description FROM (
    SELECT 'marital_status' AS module, 'view'     AS action, 'marital_status:view'     AS slug, NULL AS description
    UNION ALL SELECT 'marital_status', 'create',   'marital_status:create',   NULL
    UNION ALL SELECT 'marital_status', 'edit',     'marital_status:edit',     NULL
    UNION ALL SELECT 'marital_status', 'delete',   'marital_status:delete',   NULL
    UNION ALL SELECT 'marital_status', 'download', 'marital_status:download', NULL
) AS new_perms
WHERE NOT EXISTS (
