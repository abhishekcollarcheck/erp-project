-- ============================================================================
--  Notice Period form definition — MySQL
--  Same pattern as Employee Type/Status: one hr_modules row, one
...
--  NoticePeriodPage.tsx's UI, so excluded, same principle as every prior
--  seed script.
--
...
VALUES
    ('Notice Period', 'notice_period', '📄', 'Notice period catalog for resignations/exits', 10, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'notice_period' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Notice Period', 'notice_period', 'Notice period catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'notice_period' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Notice Period Name', 'notice_period_name', 'Notice Period',
     'Add notice period...', NULL,
...
SELECT module, action, slug, description FROM (
    SELECT 'notice_period' AS module, 'view'     AS action, 'notice_period:view'     AS slug, NULL AS description
    UNION ALL SELECT 'notice_period', 'create',   'notice_period:create',   NULL
    UNION ALL SELECT 'notice_period', 'edit',     'notice_period:edit',     NULL
    UNION ALL SELECT 'notice_period', 'delete',   'notice_period:delete',   NULL
    UNION ALL SELECT 'notice_period', 'download', 'notice_period:download', NULL
) AS new_perms
WHERE NOT EXISTS (
