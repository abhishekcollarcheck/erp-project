...
--  model but isn't exposed in GenderPage.tsx's UI, so excluded, same
--  principle as every prior seed script. max_length 50 matches
--  Gender.name (STRING(50) — shorter than the usual 100).
--
...
VALUES
    ('Gender', 'gender', '⚧️', 'Gender catalog', 14, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @module_id = (SELECT id FROM hr_modules WHERE slug = 'gender' LIMIT 1);
...
VALUES
    (NULL, @module_id, 'Gender', 'gender', 'Gender catalog entries', 1, 1, 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE updated_at = NOW();

SET @form_id = (SELECT id FROM form_definitions WHERE module_id = @module_id AND slug = 'gender' LIMIT 1);
...
VALUES
    (NULL, @form_id, 'text', 'Gender Name', 'gender_name', 'Gender',
     'Add gender...', NULL,
     1, 50, NULL, NULL, NULL,
     NULL, 0, 1, 1, NOW(), NOW())
...
SELECT module, action, slug, description FROM (
    SELECT 'gender' AS module, 'view'     AS action, 'gender:view'     AS slug, NULL AS description
    UNION ALL SELECT 'gender', 'create',   'gender:create',   NULL
    UNION ALL SELECT 'gender', 'edit',     'gender:edit',     NULL
    UNION ALL SELECT 'gender', 'delete',   'gender:delete',   NULL
    UNION ALL SELECT 'gender', 'download', 'gender:download', NULL
) AS new_perms
WHERE NOT EXISTS (
