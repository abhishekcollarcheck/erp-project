...
// `sequelize.sync({ alter: true })` (dev boot) already created this table on
// many local DBs before this migration existed, so addIndex must tolerate
// "index already exists" instead of throwing.
...
export async function up(queryInterface: QueryInterface): Promise<void> {
  ...
  await addIndexIfMissing(queryInterface, 'genders', ['display_order']);
}
