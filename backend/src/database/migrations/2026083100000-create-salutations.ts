...
export async function up(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.createTable('salutations', {
    id: {
  ...
  await addIndexIfMissing(queryInterface, 'salutations', ['display_order']);
}

export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.dropTable('salutations');
}