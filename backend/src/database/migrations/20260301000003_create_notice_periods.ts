...
export async function up(queryInterface: QueryInterface): Promise<void> {
  ...
  await addIndexIfMissing(queryInterface, 'notice_periods', ['display_order']);
  await addIndexIfMissing(queryInterface, 'notice_periods', ['is_active']);
}

export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.dropTable('notice_periods');
}