...
export async function up(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.createTable('house_types', {
    id: {
  ...
  await addIndexIfMissing(queryInterface, 'house_types', ['display_order']);
}

export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.dropTable('house_types');
}