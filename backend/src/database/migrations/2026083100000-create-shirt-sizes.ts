...
export async function up(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.createTable('shirt_sizes', {
    id: {
  ...
  await addIndexIfMissing(queryInterface, 'shirt_sizes', ['display_order']);
}

export async function down(queryInterface: QueryInterface): Promise<void> {
  await queryInterface.dropTable('shirt_sizes');
}