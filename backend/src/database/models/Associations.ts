...
LeaveRequest.hasMany(LeaveRequestDay, { foreignKey: 'leave_request_id', as: 'breakdown' });
LeaveRequestDay.belongsTo(LeaveRequest, { foreignKey: 'leave_request_id', as: 'leaveRequest' });

Employee.hasOne(EmployeeWeeklyOffAssignment,    { foreignKey: 'employee_id', as: 'weeklyOffAssignment' });
EmployeeWeeklyOffAssignment.belongsTo(Employee, { foreignKey: 'employee_id', as: 'employee'            });

LeaveRequest.belongsTo(Employee, { foreignKey: 'approved_by',  as: 'approver'    });
LeaveRequest.belongsTo(Employee, { foreignKey: 'applied_by',   as: 'appliedBy'   });
LeaveRequest.belongsTo(Employee, { foreignKey: 'cancelled_by', as: 'cancelledBy' });
LeaveRequest.belongsTo(Employee, { foreignKey: 'hod_id',       as: 'hod'         });

Employee.hasMany(EmployeeLeaveBalance,   { foreignKey: 'employee_id', as: 'leaveBalances' });
EmployeeLeaveBalance.belongsTo(Employee, { foreignKey: 'employee_id', as: 'employee'       });

Employee.hasMany(EmployeeLeaveMinutesBalance,   { foreignKey: 'employee_id', as: 'shortLeaveBalances' });
EmployeeLeaveMinutesBalance.belongsTo(Employee, { foreignKey: 'employee_id', as: 'employee'            });

Employee.hasMany(EmployeeLeaveAccrual,   { foreignKey: 'employee_id', as: 'leaveAccruals' });
EmployeeLeaveAccrual.belongsTo(Employee, { foreignKey: 'employee_id', as: 'employee'       });

Employee.hasMany(LeaveCredit,   { foreignKey: 'employee_id', as: 'leaveCredits' });
LeaveCredit.belongsTo(Employee, { foreignKey: 'employee_id', as: 'employee'     });
// credited_by is who credited it (HR/Admin), not the recipient — separate alias
LeaveCredit.belongsTo(Employee, { foreignKey: 'credited_by', as: 'creditedByEmployee' });

Company.hasOne(LeavePolicySetting,    { foreignKey: 'company_id', as: 'leavePolicy' });
LeavePolicySetting.belongsTo(Company, { foreignKey: 'company_id', as: 'company'     });
