...
type EmployeeCreation = Optional<EmployeeAttrs,
  ...
  | 'working_state_country' | 'pay_register_location' | 'shift_id' | 'shift_type'
  | 'duration'
...
export class Employee extends Model<EmployeeAttrs, EmployeeCreation> implements EmployeeAttrs {
  ...
  public shift_type!: 'shift' | 'duration';
...
Employee.init({
  ...
  shift_type: { type: DataTypes.ENUM('shift', 'duration'),  allowNull: false,  defaultValue: 'shift',  },
  duration: { type: DataTypes.DECIMAL(5, 2),  allowNull: true, },
...
export class EmployeeCommitmentProbation extends Model {
  ...
  // ==================================================
  /**
   * Total EL credited while employee was on probation.
   *
   * Example:
   * Month 1 -> 1 EL
   * Month 2 -> 1 EL
   * Month 3 -> 1 EL
   *
   * probation_el_credit = 3
   */
  public probation_el_credit!: number;
  /**
   * Total probation EL already transferred
   * to employee_leave_balances after confirmation.
   *
   * Example:
   * probation_el_credit = 6
   * probation_el_transferred = 6
   *
   * This prevents the same EL from being transferred twice.
   */
  public probation_el_transferred!: number;
}
EmployeeCommitmentProbation.init(
  {
    employee_id: {
