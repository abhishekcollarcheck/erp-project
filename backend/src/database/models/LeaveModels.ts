...
// ************************** Most Recent Backup unique correct *************************** 
...
// WeeklyOffPreset lives in its own model file.
// Import it in your associations file if you need to wire
// EmployeeWeeklyOffAssignment <-> WeeklyOffPreset associations.
//
// Holiday is not referenced directly in this model file.
// Your sandwich-day calculation service can import Holiday there.

/* ============================================================================
 * LEAVE MANAGEMENT — MODELS
 * ----------------------------------------------------------------------------
 * Covers:
 * - EL / CL / Short Leave / Special Leave / Half Day
 * - Sandwich policy
 * - Per-employee weekly-off assignment
 * - Monthly short-leave minutes tracking
 * - Special-leave crediting
 * - Per-day audit breakdown
 *
 * IMPORTANT:
 * - Do NOT use attribute-level `unique: true` for these models.
 * - All unique constraints are defined explicitly in `indexes`.
 * - Keep index names stable.
...
export type LeaveSubmissionType =
  | 'self'
  | 'admin';
 ...
 * 1. LEAVE TYPE
 * ----------------------------------------------------------------------------
 * One row per leave type for a company.
 * Examples:
 * EL, CL, SHORT, SPECIAL, HALF
 * ========================================================================== */
...
LeaveType.init(
  {
    ...
    unit: {
      type: DataTypes.ENUM('day', 'minutes'),
      allowNull: false,
      defaultValue: 'day',
    },
  ...
  },
  {
    ...
    indexes: [
      {
        name: 'leave_type_company_code_unique',
        unique: true,
...
export class LeavePolicySetting
  extends Model<
    ...
    Optional<
      ...
      | 'id'
      | 'sandwich_enabled'
      | 'sandwich_include_weekly_off'
      | 'sandwich_include_holidays'
    >
  >
  implements LeavePolicySettingAttributes {
  public id!: number;
...
LeavePolicySetting.init(
  ...
  {
    sequelize,
    tableName: 'leave_policy_settings',
    modelName: 'LeavePolicySetting',
    timestamps: true,
    underscored: true,

    indexes: [
      {
        name: 'leave_policy_setting_company_unique',
        unique: true,
        fields: ['company_id'],
      },
    ],
...
export class EmployeeWeeklyOffAssignment
  extends Model<
    EmployeeWeeklyOffAssignmentAttributes,
    Optional<EmployeeWeeklyOffAssignmentAttributes, 'id'>
  >
  implements EmployeeWeeklyOffAssignmentAttributes {
  public id!: number;
  public employee_id!: number;
...
EmployeeWeeklyOffAssignment.init(
  ...
  {
    sequelize,
    tableName: 'employee_weekly_off_assignments',
    modelName: 'EmployeeWeeklyOffAssignment',
    timestamps: true,
    underscored: true,

    indexes: [
      {
        name: 'employee_weekly_off_assignment_employee_unique',
        unique: true,
        fields: ['employee_id'],
      },
    ],
...
EmployeeLeaveBalance.init(
  ...
  },
  {
    ...
    indexes: [
      ...
      {
        name: 'employee_leave_balance_employee',
        fields: ['employee_id'],
      },

      {
        name: 'employee_leave_balance_leave_type',
        fields: ['leave_type_id'],
      },

      {
        name: 'employee_leave_balance_year',
        fields: ['year'],
      },
    ],
  },
...
/* ============================================================================
 * 5. EMPLOYEE SHORT-LEAVE MINUTES BALANCE
 * ----------------------------------------------------------------------------
 * Monthly quota.
 * ========================================================================== */
...
EmployeeLeaveMinutesBalance.init(
  ...
  },
  {
    ...
    indexes: [
      ...
      {
        name: 'employee_leave_minutes_balance_employee_year',
        fields: ['employee_id', 'year'],
      },
    ],
  },
 ...
 * 9. LEAVE REQUEST DAY
 * ----------------------------------------------------------------------------
 * Per-day audit trail for sandwich / charged days.
 * ========================================================================== */
