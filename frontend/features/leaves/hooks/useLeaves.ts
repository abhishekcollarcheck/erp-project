...
// ─── Edit ─────────────────────────────────────────────────────────────────────
// NEW — updates one of the caller's own Pending leave requests. Invalidates
// everything under 'leaves' since editing can change balances, the pending
// list (if managers changed — they don't via this hook, but the request's
// date/type did), and the specific detail/byId cache.
export function useEditLeave() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: EditLeaveDto }) => leaveService.edit(id, data),
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: KEYS.all });
      qc.invalidateQueries({ queryKey: KEYS.byId(vars.id) });
      showToast('✓ Leave request updated');
    },
    onError: (err: any) => showToast(err?.message || 'Failed to update leave request'),
  });
}
