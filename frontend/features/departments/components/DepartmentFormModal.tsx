...
import { Modal } from '../../../components/ui/Modal';
import { useCreateDepartment, useUpdateDepartment } from '../hooks/useDepartments';
import type { Department, CreateDepartmentDto, UpdateDepartmentDto } from '../../../services/api/department.service';
import { FormInput } from '../../../components/form/FormInput';
import { FormSelect } from '../../../components/form/FormSelect';
import { useFieldPermissions, resolveFieldPerm } from '../../employees/hooks/useEmployees';
import { FormSection } from '../../../components/form/FormSection';
...
export function DepartmentFormModal({ open, onClose, department, managers }: Props) {
  const { data: fp } = useFieldPermissions('department');
  const f = (n: string) => resolveFieldPerm(fp, n);
  const isEdit = !!department;
  ...
  const onSubmit = async (data: CreateDepartmentFormData) => {
    if (isEdit && department) {
      const updatePayload: UpdateDepartmentDto = {
        department_name: data.department_name,
        department_code: data.department_code || null,
        head_id: data.head_id || null,
      };
      await updateMutation.mutateAsync({ id: department.id, data: updatePayload });
    } else {
      const createPayload: CreateDepartmentDto = {
        department_name: data.department_name,
        department_code: data.department_code || null,
        head_id: data.head_id || null,
      };
