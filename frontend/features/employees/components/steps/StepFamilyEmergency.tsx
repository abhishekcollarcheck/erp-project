...
export function StepFamilyEmergency(_: Props) {
  ...
  return (
    <FormSection fields={[f('marital_status'), f('father_salutation'), f('father_name'), f('father_dob'), f('father_occupation'), f('mother_salutation'), f('mother_name'), f('mother_dob'), f('mother_occupation'), f('emergency_contacts')]}>
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr 1fr', gap: 12 }}>
        <FormSelect name="father_salutation" label="Father Salutation" options={toOpts(FATHER_SALUTATION)} placeholder="Select" fieldPerm={f('father_salutation')} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr 1fr', gap: 12 }}>
        <FormSelect name="mother_salutation" label="Mother Salutation" options={toOpts(MOTHER_SALUTATION)} placeholder="Select" fieldPerm={f('mother_salutation')} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button type="button" className="btn btn-sec btn-sm" onClick={() => familyMembers.append({ name: '', relationship: '', dob: '', occupation: '' })}>+ Add member</button>
      </div>
      ...
      {/* ── Emergency Contacts (repeatable — first is primary) ────────────── */}
      ...
      {emergencyContacts.fields.length === 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
          <FormInput name="emergency_contacts.0.contact_name" label="Contact person name" fieldPerm={f('emergency_contacts')} />
          <FormInput name="emergency_contacts.0.contact_number" label="Contact number" fieldPerm={f('emergency_contacts')} />
          <FormInput name="emergency_contacts.0.email" label="Email" type="email" placeholder="name@email.com" fieldPerm={f('emergency_contacts')} />
          <FormSelect name="emergency_contacts.0.relationship" label="Relationship" options={[...RELATIONSHIP_OPTIONS]} placeholder="Select" fieldPerm={f('emergency_contacts')} />
        </div>
      )}
      ...
      <div>
        ...
        <button type="button" className="btn btn-sec btn-sm" onClick={() => emergencyContacts.append({ contact_name: '', contact_number: '', email: '', relationship: '' })}>+ Add contact</button>
      </div>
    </div>
    </FormSection>
  );
