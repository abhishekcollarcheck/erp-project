...
import { BANK_NAME_OPTIONS, DOC_TYPE_OPTIONS } from '../../constants/employee.constants';
...
function KycCard({ title, hint, required, children }: { title: string; hint: string; required?: boolean; children: React.ReactNode }) {
  ...
  return (
    <section style={{ border: '1px solid var(--border)', borderRadius: 'var(--r2)', marginBottom: 10, overflow: 'hidden' }}>
      <button type="button" onClick={() => setOpen(o => !o)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, background: 'var(--surface2)', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        <div>
...
export function StepIdsBank(_: Props) {
  ...
  return (
    <FormSection fields={[f('aadhaar_number'), f('aadhaar_name'), f('aadhaar_dob'), f('aadhaar_address'), f('pan_number'), f('passport_number'), f('driving_license_number'), f('personal_bank_name'), f('personal_bank_account'), f('personal_ifsc'), f('personal_bank_branch')]}>
      ...
      <KycCard title="Vaccinations" hint="Select major vaccines (COVID, booster, yellow fever, and more)">
        ...
        {vaccinations.fields.map((field, i) => (
          <div key={field.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12, alignItems: 'end', marginBottom: 8 }}>
            <FormInput name={`vaccinations.${i}.vaccine_name`} label="Vaccine name" fieldPerm={f('vaccinations')} />
            <FormDatePicker name={`vaccinations.${i}.date`} label="Date" fieldPerm={f('vaccinations')} />
            <button type="button" className="btn btn-sec btn-sm" onClick={() => vaccinations.remove(i)}>Remove</button>
          </div>
        ))}
        <button type="button" className="btn btn-sec btn-sm" onClick={() => vaccinations.append({ vaccine_name: '', date: '' })}>+ Add vaccination</button>
      </KycCard>
