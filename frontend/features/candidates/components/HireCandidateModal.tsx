...
// import { useHireCandidate } from '../hooks/useCandidates';
// import { useDepartmentOptions } from '../../departments/hooks/useDepartments';
// import { useDesignations } from '../../designations/hooks/useDesignations';
// import { useEmployees } from '../../employees/hooks/useEmployees';
// import type { Candidate } from '../types/candidate.types';
...
//   designation_id: z
//     .number({ invalid_type_error: 'Designation is required' })
//     .int()
...
//   const { data: designationsList = [], isLoading: loadingDesignations } = useDesignations({
//     is_active: 'true',
//   });
...
//   // ─── Computed Options ───────────────────────────────────────────────────
//   // Filter designations matching selected department or global designations (is_all_departments)
//   const filteredDesignations = useMemo(() => {
//     if (!selectedDeptId) return [];

//     return designationsList.filter((des) => {
//       if (des.is_all_departments) return true;
//       return des.departments?.some((dept) => dept.id === selectedDeptId);
//     });
//   }, [designationsList, selectedDeptId]);
...
//         employment_type: 'Full-time',
//         date_of_joining:
//           candidate?.confirmed_joining_date?.slice(0, 10) ||
//           candidate?.decision_joining_date?.slice(0, 10) ||
//           today,
//         reporting_manager_id: null,
//       });
...
//   // Reset selected designation when department selection changes
//   useEffect(() => {
//     setValue('designation_id', undefined as any);
//   }, [selectedDeptId, setValue]);
...
//           <label>
//             Reporting Manager{' '}
//             <span style={{ textTransform: 'none', fontWeight: 400, color: 'var(--ink4)' }}>
//               — optional
//             </span>
//           </label>
//           <select {...register('reporting_manager_id', { valueAsNumber: true })}>
...
export function HireCandidateModal({ open, onClose, candidate }: Props) {
  ...
  const { data: departmentsList = [], isLoading: loadingDepartments } = useDepartments({
    is_active: 'true',
  });
  ...
  // ─── Options Formatting ────────────────────────────────────────────────
  // Map department list into options format ({ value, label })
  ...
  // Filter designations belonging to the selected department or available globally
  const filteredDesignations = useMemo(() => {
    if (!selectedDeptId) return [];
