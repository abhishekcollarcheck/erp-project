...
export interface LeaveRequest {
  ...
  applied_by?: number | null;
  applied_at?: string;
  // Manager-of-record for this specific request — set at apply()/edit() time,
  // used by the backend to decide who can approve/reject it.
...
export interface ApplyLeaveDto {
  ...
  hod_name?: string;
  coordinator_name?: string;
  // Manager-of-record, validated server-side against the employee's actual
  // L1/L2 mapping. Populate from useMyManagers() for self-service apply.
  l1_manager_id?: number | null;
  l2_manager_id?: number | null;
  undertaking_accepted: boolean;
}

// All fields optional — only send what actually changed; edit() on the
// backend falls back to the existing value for anything omitted.
...
export const leaveService = {
  ...
  // NEW — owner-only, only while the request is still Pending. See
  // leave.service.ts (backend) LeaveService.edit() for the balance-hold
  // re-validation this triggers server-side.
  edit: (id: number, data: EditLeaveDto) =>
    apiClient.put<unknown, ApiResponse<LeaveRequest>>(`/leaves/${id}`, data),
