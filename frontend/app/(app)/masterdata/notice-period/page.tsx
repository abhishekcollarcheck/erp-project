...
const FIELD_KEY = 'notice_period_name';

export default function NoticePeriodPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('notice_period');
  ...
  const canAddCurrent = canCreate('notice_period') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('notice_period') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('notice_period');
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add notice period');
    }
  };

  const startEdit = (item: NoticePeriod) => {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm('Are you sure you want to delete this notice period?')) return;
    try {
      await deleteMutation.mutateAsync(id);
      if (editingId === id) cancelEdit();
    } catch (err: any) {
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete notice period');
    }
  };

  const filteredItems = useMemo(() => {
    return noticePeriods.filter((item) => item.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [noticePeriods, filterText]);

  return (
    <PermissionGuard permission="notice_period:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Notice Period</h1>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No notice periods found.</div>
              ) : (
