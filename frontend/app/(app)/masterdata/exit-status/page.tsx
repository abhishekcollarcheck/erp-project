...
const FIELD_KEY = 'exit_status_name';

export default function ExitStatusPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('exit_status');
  ...
  const canAddCurrent = canCreate('exit_status') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('exit_status') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('exit_status');
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add exit status');
    }
  };

  const startEdit = (item: ExitStatus) => {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm('Are you sure you want to delete this exit status?')) return;
    try {
      await deleteMutation.mutateAsync(id);
      if (editingId === id) cancelEdit();
    } catch (err: any) {
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete exit status');
    }
  };

  const filteredItems = useMemo(() => {
    return exitStatuses.filter((item) => item.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [exitStatuses, filterText]);

  return (
    <PermissionGuard permission="exit_status:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No exit statuses found.</div>
              ) : (
