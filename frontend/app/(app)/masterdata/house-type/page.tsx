...
const FIELD_KEY = 'house_type_name';

export default function HouseTypePage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('house_type');
  ...
  const canAddCurrent = canCreate('house_type') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('house_type') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('house_type');
  ...
  const handleAddHouseType = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add house type');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteHouseType.mutateAsync(id);
      if (editingId === id) cancelEdit();
    } catch (err: any) {
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete house type');
    }
  };

  const filteredHouseTypes = useMemo(() => {
    return houseTypes.filter((h) => h.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [houseTypes, filterText]);

  return (
    <PermissionGuard permission="house_type:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">House Type</h1>
            ...
            {canAddCurrent && (
              <div className="flex items-center gap-2 py-4">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleAddHouseType()}
                />
                <button
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredHouseTypes.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No house types found.</div>
              ) : (
                <div className="space-y-1">
                  {filteredHouseTypes.map((item, index) => {
