...
const FIELD_KEY = 'type_name';

export default function EmployeeTypesPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('employee_type');
  ...
  const canAddCurrent = canCreate('employee_type') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('employee_type') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('employee_type');
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add type');
    }
  };

  const startEdit = (type: EmployeeType) => {
  ...
  const handleSaveEdit = async (id: number) => {
    if (!canEditCurrent || !editName.trim()) return;
    try {
      await updateType.mutateAsync({ id, data: { name: editName.trim() } });
      cancelEdit();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete type');
    }
  };

  const filteredTypes = useMemo(() => {
    return (types || []).filter((t) => t.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [types, filterText]);

  return (
    <PermissionGuard permission="employee_type:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Employee Type</h1>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredTypes.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {isLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No types found.</div>
              ) : (
