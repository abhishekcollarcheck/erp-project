...
const FIELD_KEY = 'salutation_name';

export default function SalutationPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('salutation');
  ...
  const canAddCurrent = canCreate('salutation') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('salutation') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('salutation');
  ...
  const handleAddSalutation = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add salutation');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm('Are you sure you want to delete this salutation?')) return;
    try {
      await deleteSalutation.mutateAsync(id);
      if (editingId === id) cancelEdit();
    } catch (err: any) {
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete salutation');
    }
  };

  const filteredSalutations = useMemo(() => {
    return salutations.filter((s) => s.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [salutations, filterText]);

  return (
    <PermissionGuard permission="salutation:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredSalutations.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No salutations found.</div>
              ) : (
                <div className="space-y-1">
                  {filteredSalutations.map((item, index) => {
