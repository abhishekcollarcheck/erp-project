...
const FIELD_KEY = 'emergency_relationship_name';

export default function EmergencyRelationshipPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('emergency_relationship');
  ...
  const canAddCurrent = canCreate('emergency_relationship') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('emergency_relationship') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('emergency_relationship');
  ...
  const handleAddRelationship = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add relationship');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete relationship');
    }
  };

  const filteredRelationships = useMemo(() => {
    return relationships.filter((r) => r.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [relationships, filterText]);

  return (
    <PermissionGuard permission="emergency_relationship:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Emergency Relationship</h1>
            ...
            {canAddCurrent && (
              <div className="flex items-center gap-2 py-4">
                <input
                  ...
                  placeholder="Add emergency relationship..."
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredRelationships.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No relationships found.</div>
              ) : (
