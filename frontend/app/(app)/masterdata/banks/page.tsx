...
const FIELD_KEY = 'bank_name';

export default function AllBanksPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('bank');
  ...
  const canAddCurrent = canCreate('bank') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('bank') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('bank');
  ...
  const handleAddBank = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add bank');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete bank');
    }
  };

  const filteredBanks = useMemo(() => {
    return banks.filter((b) => b.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [banks, filterText]);

  return (
    <PermissionGuard permission="bank:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredBanks.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No banks found.</div>
              ) : (
