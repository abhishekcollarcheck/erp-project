...
const FIELD_KEY = 'qualification_name';

export default function QualificationPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('qualification');
  ...
  const canAddCurrent = canCreate('qualification') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('qualification') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('qualification');
  ...
  const handleAddQualification = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add qualification');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete qualification');
    }
  };

  const filteredQualifications = useMemo(() => {
    return qualifications.filter((q) => q.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [qualifications, filterText]);

  return (
    <PermissionGuard permission="qualification:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Qualification</h1>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredQualifications.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No qualifications found.</div>
              ) : (
