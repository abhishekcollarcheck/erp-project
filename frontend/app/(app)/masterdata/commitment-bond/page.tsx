...
const FIELD_KEY = 'bond_name';

export default function BondPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('bond');
  ...
  const canAddCurrent = canCreate('bond') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('bond') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('bond');
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add bond option');
    }
  };

  const startEdit = (item: Bond) => {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm('Are you sure you want to delete this bond option?')) return;
    try {
      await deleteMutation.mutateAsync(id);
      if (editingId === id) cancelEdit();
    } catch (err: any) {
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete bond option');
    }
  };

  const filteredItems = useMemo(() => {
    return bonds.filter((item) => item.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [bonds, filterText]);

  return (
    <PermissionGuard permission="bond:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Bond</h1>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No bond options found.</div>
              ) : (
