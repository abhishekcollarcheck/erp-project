...
const FIELD_KEY = 'gender_name';

export default function GenderPage() {
  const { canCreate, canEdit, canDelete, isSuperAdmin } = usePermission();
  const { data: fp, isLoading: fieldsLoading } = useFieldPermissions('gender');
  ...
  const canAddCurrent = canCreate('gender') && fCreate(FIELD_KEY).can_edit;
  const canEditCurrent = canEdit('gender') && fieldPerm.can_edit;
  const canDeleteCurrent = canDelete('gender');
  ...
  const handleAddGender = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to add gender');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || err?.message || 'Failed to delete gender');
    }
  };

  const filteredGenders = useMemo(() => {
    return genders.filter((g) => g.name.toLowerCase().includes(filterText.toLowerCase().trim()));
  }, [genders, filterText]);

  return (
    <PermissionGuard permission="gender:view">
      <AppShell>
        <MasterDataLayout>
          <div className="flex h-full w-full flex-col bg-white p-6 font-sans text-gray-800">
            <div className="flex items-center justify-between pb-3">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-gray-900">Gender</h1>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{filteredGenders.length}</span>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-2">
              {fieldsLoading ? (
                ...
                <div className="py-8 text-center text-xs text-gray-400">No genders found.</div>
              ) : (
